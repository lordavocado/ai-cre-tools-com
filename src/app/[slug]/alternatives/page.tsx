...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: { canonical: `${siteConfig.url}/${slug}/alternatives` },
    openGraph: { title, description, url: `${siteConfig.url}/${slug}/alternatives` },
  };
}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            url: `${siteConfig.url}/${slug}/alternatives`,
            mainEntity: {
              '@type': 'ItemList',
      ...
      <section className="border-b border-[#e0e0e0] bg-white py-12 md:py-16">
        <div className="container px-6">
          <nav className="mb-6 text-sm text-[#737373]">
            ...
            <Link href={`/${slug}`} className="hover:text-[#1f1f1f]">{item.name}</Link>
            <span className="mx-1.5">/</span>
